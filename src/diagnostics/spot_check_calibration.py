...
def spot_check_frame(frame_path: Path, calib_models, detector):
    frame = cv2.imread(str(frame_path))
    if frame is None:
        print(f"[스킵] 이미지를 못 읽었습니다: {frame_path}")
        return

    H = get_homography_matrix(frame, calib_models)
    if H is None:
...
def main():
    ...
    if not frame_paths:
        print("frames/ 폴더에 이미지가 없습니다. 정면/측면/줌인 프레임을 몇 장 넣어주세요.")
        return
    ...
    for fp in frame_paths:
        spot_check_frame(fp, calib_models, detector)
