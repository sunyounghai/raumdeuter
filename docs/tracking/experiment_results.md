...
## 추가 검증: 다중 시퀀스(actionClass 계층화)

위 한계 1번의 단일 클립 기준에 대한 후속 검증

### 실행 조건
- h250_baseline + BoT-SORT 조합을 그대로 actionClass 계층화로 고른 5개 SoccerNet 시퀀스에 반복 적용
- GT는 각 시퀀스의 gameinfo.ini에서 player+goalkeeper만 직접 카운트(시퀀스마다 실제 인원이 다름)

### 결과

| 시퀀스 | actionClass | 그룹 | GT(player+GK) | 예측 트랙 수 | 배율 |
|---|---|---|---|---|---|
| SNMOT-116 | Corner | 혼잡 | 24 | 57 | 2.38배 |
| SNMOT-144 | Penalty | 혼잡 | 21 | 39 | 1.86배 |
| SNMOT-122 | Direct free-kick | 혼잡 | 22 | 62 | 2.82배 |
| SNMOT-119 | Clearance | 분산 | 22 | 34 | 1.55배 |
| SNMOT-124 | Kick-off | 분산 | 22 | 51 | 2.32배 |

### 관찰

**클립 간 배율 차이는 있으나(1.55~2.82배) 우려했던 만큼 크지는 않음**
- 혼잡군(116/144/122) 평균 2.35배, 분산군(119/124) 평균 1.94배 - 방향은 일치하나 격차는 크지 않아, actionClass만으로 트래커 성능을 예측하기는 이름